...
//  Delay_1ms(50);//10uf 断电重启 VDD、VPP掉电需要50ms
//  Delay_1ms(500);//100uf 断电重启 VDD、VPP掉电需要>500ms
//  Delay_1ms(1200);//470uf 断电重启 VDD、VPP掉电需要500ms
  if(WriteFlag)//在线烧录
  {
    Delay_1ms(1200);
  }
...
/*  
  ...
  temp=0x3c00+adjH;
  MC321_IRC_INST(temp);
  temp=0x5600+0x1fd;//0x1fd opt2 H地址
